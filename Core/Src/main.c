...
float Left_Roll_Pos = -5.6875, Right_Roll_Pos = -1.5+3, Right_Pitch_Pos = 1.5, Left_Pitch_Pos=-0.6875 , Left_Column_Error =0 , Left_Col_Pos = 0;
...
uint16_t FL_Home_Pos = 692 , FR_Home_Pos = 0, RL_Home_Pos = 0, RR_Home_Pos = 0;
...
int Flaps_Target = 48, Flap_Error=0;
...
uint8_t Save_Value = 10;
int Left_Var_Speed=5;
...
float KMPHtoRPS(float kmph)
{
    ...
    rps = (((kmph/(Circumference/1000)*1000)/3600)*300);
    return rps;
}
...
//	if ( Left_Arm_Motor_Value > 100 || Left_Arm_Motor_Value < -100 ) Error_Handler();
//	if ( Right_Arm_Motor_Value > 100 || Right_Arm_Motor_Value < -100 ) Error_Handler();
//	if ( Pitch_Arm_Motor_Value > 100 || Pitch_Arm_Motor_Value < -100 ) Error_Handler();
//	if ( Lower_Width_Motor_Value > 1000 || Lower_Width_Motor_Value < -1000 ) Error_Handler();
//	if ( Upper_Width_Motor_Value > 1000 || Upper_Width_Motor_Value < -1000 ) Error_Handler();
...
//  LP_TasksHandle = osThreadCreate(osThread(LP_Tasks), NULL);
 ...
 // osKernelStart();
  ...
  while (1)
  {
	...
	HAL_Delay(2000);
		Save_Value++;
		Left_Arm_Motor_Value = Save_Value;
	Right_Arm_Motor_Value = Save_Value;
  Pitch_Arm_Motor_Value = Save_Value;
Lower_Width_Motor_Value = Save_Value;	
Upper_Width_Motor_Value = Save_Value;	*/
    /* USER CODE END WHILE */
...
void Set_Motor_Torque ( uint8_t Axis , float Torque )
{
	Torque =  (Axis==1)|| (Axis==0) || (Axis==3)? -Torque : Torque ;	
...
void Joystick_Reception(void)
{
	...
	else { for ( uint8_t i = 0 ; i < 8 ; i++ ) {BT_Rx[i]=0;}}
}
void Manual_Controls (void)
{
	/*------------------------------WHEEL_MOTORS----------------------------------- */
...
void Z_Top_Flap_Sensing(void)
{
	 ...
	 Front_Left_Bush  =  FL_Angle > 35 ? 1 : 0;
...
//		if ( Joystick == 1 )
//		{
//		if ( Motor_Velocity[3] > 10 ) Vel_Limit = Speed*30;
//		}
//		else if ( Joystick == 2 )
//		{
//		if ( Motor_Velocity[3] < -10 ) Vel_Limit = Speed*30;
//		}
		...
		Vel_Limit = Joystick == 0 ? 15 : Speed*15;
		Vel_Limit = Vel_Limit > 65 ? 65 : Vel_Limit;
...
void Left_Frame_Controls (void)
{
		Left_Vertical_Error =  Left_Roll_Pos - L_Roll ; 
	
		Left_Var_Speed = Speed * 5;
      	
		if ( Left_Vertical_Error > 2 )  Left_Frame_Speed = Joystick == 2 ? Left_Var_Speed: -Left_Var_Speed  ;//(Left_Vertical_Error * Width_Kp) ;
	
		else if ( Left_Vertical_Error < -2 )  Left_Frame_Speed = Joystick == 2 ? -Left_Var_Speed : Left_Var_Speed;//(Left_Vertical_Error * Width_Kp) ;
